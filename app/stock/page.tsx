...
export default function Stock() {
    ...
    async function handleDelete(suppr: string) {
        await fetch(`/api/stock?suppr=${suppr}`, {
            method: "DELETE",
        });
                        ...
                        {produits.map((produit) => (
                            <tr key={produit.numero_de_serie} className="border-b hover:bg-gray-50">
                                ...
                                <td className="p-3 text-right">
                                    <button 
                                        onClick={() => handleDelete(produit.numero_de_serie)}
                                        className="bg-red-100 text-red-600 hover:bg-red-600 hover:text-white px-3 py-1 rounded transition-colors text-sm font-bold"
                                    >
                                        Supprimer
                                    </button>
                                </td>
